...
environment.debug:  ## Prints the values of the environemnt variables to be used in the make command as define in .env.* files.
	...
	@echo -e "${cyan}TAHOE_HOST_NAME${normal}       $(TAHOE_HOST_NAME)"
...
local.ssh.config: ve/bin/ansible-playbook
	@echo -e "Updating necessary records in SSH related files...    [~/.ssh/config, ~/.ssh/knwon_hosts]"
...
local.hosts.update: ve/bin/ansible-playbook  ## Updates your hosts file by adding the necessary hosts to it.
	...
	@. ve/bin/activate; sudo ansible-playbook \
		...
		-i '127.0.0.1,' \
		--tags hosts_update \
		-e "IP_ADDRESS=$(IP_ADDRESS) TAHOE_HOST_NAME=$(TAHOE_HOST_NAME)" local.yml > $(SHELL_OUTPUT) || (echo -e "${redbold}ERROR configuring hosts records.${normal}" && make error && exit 1)
...
local.hosts.revert: ve/bin/ansible-playbook  ## Updates your hosts file by removing the added hosts from it.
	@echo "Reverting made local changes...    [/etc/hosts, ~/.ssh/config]"
	@make sudocheck
	@. ve/bin/activate; sudo ansible-playbook \
		--connection=local \
		-i '127.0.0.1,' \
